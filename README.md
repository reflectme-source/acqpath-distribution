...
Documentation deployment target: [developers.getacqpath.com](https://developers.getacqpath.com), using a separate Cloudflare Pages project. See [deployment setup](docs/CLOUDFLARE-DEPLOYMENT.md) and [actual acceptance status](docs/ACCEPTANCE.md); the configured hostname alone is not evidence of a deployment.
...
`BUILT_LOCALLY` is not `PUBLISHED`. A successful submission is not confirmed indexing. A matching catalog entry is not verified purchase compatibility or organic revenue. Account-authorized publishing and a separately approved real purchase have **not been executed by the package author**.

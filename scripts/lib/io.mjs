...
export async function walk(root,skip=new Set(['.git','.private','.local','.tools','.wrangler','out','node_modules'])){const result=[];async function scan(dir){for(const e of await readdir(dir,{withFileTypes:true})){if(skip.has(e.name))continue;const p=join(dir,e.name);if(e.isSymbolicLink())throw Error('SYMLINK_IN_PACKAGE');if(e.isDirectory())await scan(p);else if(e.isFile())result.push(p);}}await scan(root);return result.sort();}

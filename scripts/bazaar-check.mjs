...
export async function validatePublicContract(root=ROOT){
 ...
 if(d.productionDeployed!==true||d.productionEndpointActive!==true||d.status!=='PRODUCTION_DISCOVERY_VERIFIED_PURCHASE_BLOCKED'||!p||p.version!=='6f17ddad-8461-4b78-b6c3-ea7b77339cc8'||p.sourceSha256!==d.coreSourceSha256||p.health!=='PASS'||p.legacyCompatibility!=='PASS'||p.unpaidDiscovery!=='PASS'||p.cdpValidator?.valid!==true||p.cdpValidator?.simulationAccepted!==true||p.preparedPurchase!=='BLOCKED_NO_VERIFIABLE_DECLARATION')throw Error('UNPROVEN_PRODUCTION_CLAIM');

...
export const QUERIES=['AcqPath','RSL rights','AI usage rights','crawl rights','RAG ingestion rights','AI training rights'];
...
export function externalBaseline(aggregate,events=[],excluded=[]){
 const deny=new Set(excluded.map(x=>x.toLowerCase())),seen=new Set(),accepted=[];
 for(const e of events){if(e.network!=='eip155:8453'||e.classification!=='EXTERNAL_VERIFIED'||e.independenceVerified!==true||e.settlementVerified!==true||e.reportDeliveryVerified!==true||e.noDuplicateChargeVerified!==true||!/^0x[a-f0-9]{64}$/i.test(e.transaction||'')||!/^0x[a-f0-9]{40}$/i.test(e.payer||'')||deny.has(e.payer.toLowerCase())||!['20000','50000'].includes(e.amountMicro)||seen.has(e.transaction.toLowerCase()))continue;seen.add(e.transaction.toLowerCase());accepted.push(e);}
 const counts=new Map();for(const e of accepted)counts.set(e.payer.toLowerCase(),(counts.get(e.payer.toLowerCase())||0)+1);
 const zero=aggregate?.mainnetPaidReports===0&&aggregate?.mainnetReceivedMicro==='0';
 return {externalPaidReports:zero?0:null,repeatExternalPayers:zero?0:null,externalRevenueMicro:zero?'0':null,verifiedExternalPaidReports:accepted.length,verifiedIndependentPayers:counts.size,verifiedRepeatPayers:[...counts.values()].filter(n=>n>1).length,verifiedExternalRevenueMicro:accepted.reduce((n,e)=>n+BigInt(e.amountMicro),0n).toString(),coverage:zero?'ZERO_MAINNET_AGGREGATE':'PARTIAL_OR_UNKNOWN',freshVerified:accepted.filter(e=>e.amountMicro==='20000').length,deepVerified:accepted.filter(e=>e.amountMicro==='50000').length,billableUnknownRatio:null,triggers:{FIRST_EXTERNAL_PAYMENT:accepted.length>0,FIRST_REPEAT_PAYER:[...counts.values()].some(n=>n>1),'10_EXTERNAL_PAID_REPORTS':accepted.length>=10,'3_INDEPENDENT_PAYERS':counts.size>=3}};
}
...
export async function monitor({localAggregates=false}={}){
 ...
 const probe=async(name,url)=>{try{const r=await json(url);const rows=r.body.resources||r.body.items||[];const matched=rows.filter(x=>(typeof x.resource==='string'?x.resource:x.resource?.url)===endpoint).map(x=>({resource:endpoint,terms:x.accepts,hasSchema:!!x.extensions?.bazaar?.schema}));checks.push({name,http:r.http,matched,partial:r.body.partialResults===true});}catch{checks.push({name,http:null,unavailable:true,matched:[]})}};
 ...
 const found=checks.some(c=>c.matched?.some(x=>x.terms?.some(a=>a.network===cfg.payment.network&&a.payTo?.toLowerCase()===cfg.payment.payTo.toLowerCase()&&a.asset?.toLowerCase()===cfg.payment.asset.toLowerCase()&&['20000','50000'].includes(a.amount))));
 const report={at:new Date().toISOString(),checks,aggregate,baseline,bazaar:found?'LISTED_READBACK':'AWAITING FIRST EXTERNAL SETTLEMENT',agenticMarket:found?'BAZAAR_SOURCE_LISTED_UI_READBACK_REQUIRED':'AWAITING FIRST EXTERNAL SETTLEMENT',externalSettlementProven:baseline.verifiedExternalPaidReports>0,noPaymentPerformed:true,noQuoteCreated:true};
